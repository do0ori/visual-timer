            ...
            >
                <div className="flex items-center gap-2 p-3 sm:p-3.5">
                    <button
                        ...
                        className="shrink-0 cursor-grab rounded-xl p-2 text-zinc-400 transition-colors hover:bg-black/5 hover:text-zinc-700 active:cursor-grabbing"
                    >
                ...
                {isOpen && (
                    <div className="space-y-5 border-t border-black/5 px-4 pb-5 pt-4 sm:px-5">
                        <div className="grid grid-cols-1 gap-5 md:grid-cols-[minmax(12rem,0.85fr)_minmax(0,1.15fr)] md:items-center">
                            <div className={`${editorDialContainerClassName} min-h-56`}>
                                <span className="mb-2 text-xs font-bold uppercase tracking-wider opacity-70">
                                    Interactive Dial Preview (Drag or Click)
                                </span>
                                <div className="size-52 sm:size-56">
                                    <TimeSelector
                                        time={item.time}
                                        currentTheme={stepTheme}
                                        setTime={(time) => setValue(`items.${index}.time`, time)}
                                        text={item.title}
                                    />
                                </div>
                                <div className="mt-3 text-center">
                                    <span className="font-display text-xl font-bold">
                                        {item.time} {item.isMinutes ? 'Minutes' : 'Seconds'}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <TimerUnitSelector
                                        ...
                                        onChange={(nextIsMinutes) =>
                                            setValue(`items.${index}.isMinutes`, nextIsMinutes)
                                        }
                                    />
                                </div>
